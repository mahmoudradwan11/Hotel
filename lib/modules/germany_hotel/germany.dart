...
class GermanyHotel extends StatelessWidget {
  const GermanyHotel({Key? key}) : super(key: key);
        ...
        builder: (context, state) {
          ...
          return Scaffold(
            ...
            appBar: AppBar(
              ...
              title: const Text('Germany Hotels',
                style: TextStyle(color: Colors.white, fontSize: 20,fontFamily: 'Jannah'),),),
            body: ListView.separated(
                itemBuilder:(context,index)=>buildCostomHotel(cubit.germanyHotels[index], context),
                separatorBuilder:(context,index)=>const SizedBox(
                  height: 1,
                ),
                itemCount: cubit.germanyHotels.length
            ),
          );
        }
    );
