...
class EgyptHotel extends StatelessWidget {
  const EgyptHotel({Key? key}) : super(key: key);
        ...
        builder: (context, state) {
          ...
          return Scaffold(
            ...
            appBar: AppBar(
              ...
              title: const Text('Egypt Hotels',
                style: TextStyle(color: Colors.white, fontSize: 20,fontFamily: 'Jannah'),),),
            body:ListView.separated(itemBuilder:(context,index)=>buildCostomHotel(cubit.egyptHotels[index],context),
                separatorBuilder:(context,index)=>const SizedBox(
                  height: 0,
                ),
                itemCount: cubit.egyptHotels.length
            ),
          );
        }
    );
